...
const ReportLostPetForm: React.FC<ReportLostPetFormProps> = ({ onClose, onSuccess }) => {
    // State management and logic remain the same
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Lost pet reported successfully. An admin will review it shortly.");
            onSuccess();
            onClose();
        } catch (error) {
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex transform transition-all overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="w-2/3 flex flex-col">
                    <div className="flex items-center justify-between p-6 border-b">
                        <h3 className="text-2xl font-semibold text-gray-800">Report Pet Details</h3>
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
                        <fieldset>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div><label htmlFor="name">Pet Name</label><input type="text" name="name" id="name" value={formData.pet.name} onChange={handlePetChange} className="form-input" required /></div>
                                <div>
                                        <label htmlFor="pet_type">Pet Type (e.g., Dog)</label>
                                        <input
                                            type="text"
                                            name="pet_type"
                                            id="pet_type"
                                            // This handles both string and object types safely
                                            value={typeof formData.pet.pet_type === 'object' ? formData.pet.pet_type.type : formData.pet.pet_type || ''}
                        ...
                        <fieldset>
                            <legend className="text-lg font-semibold text-gray-700 flex items-center mb-3"><MapPin className="w-5 h-5 mr-2 text-orange-500" /> Last Known Location</legend>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <fieldset>
                            ...
                            <div className="p-4 bg-gray-50 rounded-lg border space-y-4">
                                ...
                                {formData.pet.is_diseased && <div className="grid grid-cols-3 gap-4 pt-4 border-t"><label>Disease Name<input type="text" name="disease_name" value={formData.medical_history?.disease_name} onChange={handleMedicalChange} className="form-input" /></label><label>Stage<input type="text" name="stage" value={formData.medical_history?.stage} onChange={handleMedicalChange} className="form-input" /></label><label>Duration<input type="text" name="no_of_years" value={formData.medical_history?.no_of_years} placeholder="e.g., 2 years" onChange={handleMedicalChange} className="form-input" /></label></div>}
                            </div>
                        </fieldset>
                    </form>
                    ...
                    <div className="flex items-center justify-end p-6 border-t mt-auto">
                        ...
                        <button type="submit" onClick={handleSubmit} disabled={submitting} className="ml-3 px-5 py-2.5 text-sm font-medium text-white bg-orange-500 rounded-lg hover:bg-orange-600 disabled:bg-orange-300">{submitting ? 'Saving...' : 'Submit Report'}</button>
                    </div>
                </div>
